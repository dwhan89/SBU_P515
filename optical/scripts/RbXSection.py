...
# plot I/I_0 vs. n 
plt.errorbar(n_data,Inormlog_data,xerr=n_err,color='black',fmt='.',label='data')
plt.plot(x,linear(x,*popt),label='fit')
plt.ylabel(r"$\ln{\left(\frac{I}{I_{0}}\right)}$",rotation=0,size=18,labelpad=25,fontweight='bold')
plt.xlabel(r"\textbf{Number Density} $[m^{-3}]$",size=18)
plt.text(2.25e16,-0.075,r'\textbf{slope = %.2f $\pm$ %.2f} $\times10^{-18}$ $m^{3}$' % (popt[1]*1.0e18,perr[1]*1.0e18), ha='center', va='center',fontsize=16,fontweight='bold')
plt.text(2.25e16,-0.085,r'\textbf{$\sigma$ = %.2f $\pm$ %.2f} $\times10^{-17}$ $m^{2}$' % ((popt[1]/0.05)*-1.0e17,(perr[1]/0.05)*1.0e17),ha='center',va='center',fontsize=16,fontweight='bold')
plt.legend()
